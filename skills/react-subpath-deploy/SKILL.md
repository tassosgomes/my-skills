...
description: >
  Configura projetos React (Vite) para deploy em subpath no Kubernetes, permitindo múltiplas POCs
  compartilharem um único host (ex: host/poc-01, /poc-02). Use sempre que o contexto envolver:
  deploy de SPA React em subpath, base path no Vite, React Router basename, Nginx SPA fallback em
  subpath, Dockerfile multi-stage para React com path dinâmico, Ingress com múltiplos paths, ou
  variações de "rodar frontend em /meu-path em vez da raiz". Dispare com "multipath", "subpath",
  "base path", "compartilhar host", "deploy de POC", "múltiplas POCs no mesmo domínio", ou ao criar
  projeto React para Kubernetes com path diferente de "/". Gera Dockerfile, nginx.conf.template,
  Ingress e ajusta vite.config.ts e React Router.
---
