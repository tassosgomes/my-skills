...
## Access method — check in order

**Before running anything**, determine which access method is available:

### Priority 1 — Context7 MCP (preferred)

Check if the tools `mcp__plugin_context7_context7__resolve-library-id` and `mcp__plugin_context7_context7__query-docs` are available in your current tool list.

If they are, use them directly — no CLI required:
...
mcp__plugin_context7_context7__query-docs  { context7CompatibleLibraryID: "<id>", topic: "<query>" }
```

### Priority 2 — ctx7 CLI (fallback)

Use this only if the MCP tools are **not** available. Make sure the CLI is up to date:
...
Two-step process regardless of access method: resolve the library name to an ID, then query docs with that ID.

```bash
# CLI — Step 1: Resolve library ID
ctx7 library <name> <query>

# CLI — Step 2: Query documentation
ctx7 docs <libraryId> <query>
```

You MUST resolve the library ID first to obtain a valid ID UNLESS the user explicitly provides one in the format `/org/project` or `/org/project/version`.
